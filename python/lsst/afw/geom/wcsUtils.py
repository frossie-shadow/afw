...
from lsst.afw.geom import Point2D, linearizeTransform, makeTransform
...
def makeDistortedTanWcs(tanWcs, pixelToFocalPlane, focalPlaneToPupil):
    ...
    skyFrame = wcs.getFrame()
    assert skyFrame.getClass() == "SkyFrame"
    tanSkyIndex = wcs.getCurrent()

    # make sure that a SkyFrame with domain SKYWCS does not already exist
    template = ast.SkyFrame("Domain=TANSKY")
    if wcs.findFrame(template) is not None:
        raise RuntimeError("tanWcs already has distortion; it contains a sky frame with Domain=TANWCS")

    skyFrame.setDomain("SKY")  # a copy to use for sky with distortion
    wcs.setCurrent(tanSkyIndex)
    wcs.setDomain("TANSKY")

    pixelIndex = findFrame2ByDomain(wcs, "PIXEL0")
    gridIndex = findFrame2ByDomain(wcs, "GRID")
    tanIwcIndex = findFrame2ByDomain(wcs, "IWC")
    iwcFrame = wcs.getFrame(tanIwcIndex)  # a copy to use for IWC with distortion
    wcs.setDomain("TANIWC")

    pixelToGrid = wcs.getMapping(pixelIndex, gridIndex)
    gridToIwc = wcs.getMapping(gridIndex, tanIwcIndex)
    iwcToSky = wcs.getMapping(tanIwcIndex, tanSkyIndex)
    ...
    wcs.addMapping(gridIndex, gridToDistortedGrid.then(gridToIwc), iwcFrame)
    wcs.addMapping(ast.FrameSet.CURRENT, iwcToSky, skyFrame)
    return wcs
