...
public:
     ...
     * @param fromCoordSys, toCoordSys coordinate systems between which to transform
     * @returns the transformed value. Equivalent to
     *          `getTransform(fromCoordSys, toCoordSys).applyForward(point)`.
     *
     * @throws lsst::pex::exceptions::InvalidParameterError Thrown if either
     *         `fromCoordSys` or `toCoordSys` is not supported.
     */
    geom::Point2D transform(geom::Point2D const &point, CameraSys const &fromCoordSys,
    ...
    std::vector<geom::Point2D> transform(std::vector<geom::Point2D> const &pointList,
                                         CameraSys const &fromCoordSys, CameraSys const &toCoordSys) const;
    ...
    /**
     * Get a Transform from one coordinate system to another.
     *
     * @param fromCoordSys, toCoordSys coordinate systems between which to transform
     * @returns an invertible Transform converting from `fromCoordSys` to `toCoordSys`.
     *
     * @throws lsst::pex::exceptions::InvalidParameterError Thrown if either
     *         `fromCoordSys` or `toCoordSys` is not supported.
     */
    std::shared_ptr<Transform> getTransform(CameraSys const &fromCoordSys, CameraSys const &toCoordSys) const;
...
private:
     ...
     *
     * @param fromCoordSys, toCoordSys coordinate systems between which to transform
     * @return an invertible Mapping that converts from `fromCoordSys` to `toCoordSys`
     *
     * @throws lsst::pex::exceptions::InvalidParameterError Thrown if either
     *         `fromCoordSys` or `toCoordSys` is not supported.
     */
    std::shared_ptr<ast::Mapping const> _getMapping(CameraSys const &fromCoordSys,
                                                    CameraSys const &toCoordSys) const;
